...
import { isCurrentDataApply } from "../workbench/workspace-state.js";

export function IndexDesignerModal({ session, workspaceId, tableRef, info, onDone, onClose }) {
    ...
    const create = async () => {
        ...
        if (!entry)
            return;
        const ownership = { workspaceId, scopeEpoch: session.scopeGeneration || 0, generation: entry.generation };
        const operationCtx = { ...session.ctx };
        try {
            await session.driver.runQuery(operationCtx, sql, { timeoutMs: session.timeoutMs });
            if (!isCurrentDataApply(session, ownership))
                return;
